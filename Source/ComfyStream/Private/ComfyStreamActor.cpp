...
void AComfyStreamActor::OnRGBATextureReceived(UTexture2D* Texture)
{
	...
	int32 ObjectNumber = (SequenceCounter / 3) + 1;  // Which object (1-based)
	SequenceCounter++;
	
	UE_LOG(LogTemp, Warning, TEXT("[ComfyStreamActor] ═══ IMAGE #%d (Object %d, Type: %s) %dx%d ═══"), 
		SequenceCounter, ObjectNumber, 
	...
	if (bEnable3DReconstruction && ImageIndex == 2)
	{
		...
		if (LatestRGBATexture && LatestDepthTexture && LatestMaskTexture)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("[ComfyStreamActor] ████ RECONSTRUCTION TRIGGERED for Object %d ████"), ObjectNumber);
				UE_LOG(LogTemp, Display, TEXT("[ComfyStreamActor] Time since last: %.4fs (min: %.4fs)"), 
			...
			{
				UE_LOG(LogTemp, Error, TEXT("[ComfyStreamActor] ❌ DEBOUNCED Object %d: Only %.4fs since last reconstruction (min: %.4fs)"), 
					ObjectNumber, TimeSinceLastReconstruction, MinReconstructionInterval);
			}
		}
		else
		{
	...
	else if (bEnable3DReconstruction && ImageIndex != 2)
	{
		...
		UE_LOG(LogTemp, Display, TEXT("[ComfyStreamActor] Object %d: Waiting for complete set... RGB:%s Depth:%s Mask:%s"), 
			ObjectNumber,
			LatestRGBATexture ? TEXT("YES") : TEXT("NO"),
			LatestDepthTexture ? TEXT("YES") : TEXT("NO"), 
			LatestMaskTexture ? TEXT("YES") : TEXT("NO"));
...
void AComfyStreamActor::Perform3DReconstruction()
{
	...
	if (bAccumulateObjects)
	{
		...
		// Use ONLY depth-based offset (no ObjectIndex multiplier to prevent images from moving backwards)
		float X = -DepthBasedDistance;  // Negative X = towards camera
	...
	for (FObjectData& Obj : Objects)
	{
		...
		if (SpawnedActor)
		{
			...
			// Start timer to destroy the object after 1 second
			...
			GetWorld()->GetTimerManager().SetTimer(
				...
				[this, SpawnedActor]()
				{
					if (SpawnedActor && SpawnedActor->IsValidLowLevel())
					{
						...
						// CRITICAL: Remove from array to prevent accumulation
						SpawnedObjects.Remove(SpawnedActor);
						UE_LOG(LogTemp, Display, TEXT("[ComfyStreamActor] Object destroyed, %d objects remaining"), SpawnedObjects.Num());
					}
				},
				1.0f,
				false
